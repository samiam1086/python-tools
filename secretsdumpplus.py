...
class DumpSecrets:
    def __init__(self, ip, username='', password='', domain='', options=None):
        ...
        self.__remoteName = ip
        self.__remoteHost = ip
...
if __name__ == '__main__':
    ...
    if options.hashes is not None and options.hashes.find(':') == -1:
        options.hashes = ':' + options.hashes

    addresses = do_ip(remoteName, get_ip())

    os.makedirs('{}/loot/{}'.format(cwd, timestamp))
